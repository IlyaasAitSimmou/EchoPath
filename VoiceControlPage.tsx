...
export default function VoiceControlPage() {
  ...
  const passiveLocationWatchRef = useRef<Location.LocationSubscription | null>(
    null,
  );
  ...
  const getCurrentLocationWithPermission = async (): Promise<LatLon | null> => {
    ...
    if (!hasPermission) {
      setLastGuidance(
        "Location permission is required. Please allow it in the prompt.",
      );
    ...
    if (!servicesEnabled) {
      setLastGuidance(
        "Location services are off. Turn on device location/GPS.",
      );
    ...
    try {
      ...
      setLastGuidance(
        "Waiting for GPS fix. Set emulator location or move outdoors.",
      );
      return null;
    }
  };
  ...
  const handleLocationUpdate = (nextLocation: LatLon) => {
    ...
    if (
      ...
      currentStepIndexRef.current >= routeStepsRef.current.length
    ) {
      return;
    }

    const nextStep = routeStepsRef.current[currentStepIndexRef.current];
    const distanceToStep = distanceMetersBetween(
      nextLocation,
      nextStep.location,
    );
    ...
    const initLocation = async () => {
      try {
        ...
        passiveLocationWatchRef.current = await Location.watchPositionAsync(
          ...
          (nextPosition: Location.LocationObject) => {
            setLocationFromCoords(nextPosition.coords);
          },
        );
      } catch {
        setLastGuidance(
          "Waiting for GPS fix. Set emulator location or check GPS settings.",
        );
      }
    };
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.value}>
        Step:{" "}
        {routeSteps.length
          ? `${Math.min(currentStepIndex + 1, routeSteps.length)}/${routeSteps.length}`
          : "-"}
      </Text>
      <Text style={styles.value}>
        Location:{" "}
      ...
      <Text style={styles.value}>
        ...
        {currentLocationAccuracyMeters !== null
          ? `${Math.round(currentLocationAccuracyMeters)} m`
          : "Unknown"}
      </Text>
      ...
      <Text style={styles.hint}>
        Commands: start navigation, stop navigation, repeat, help, set
        destination to ...
      </Text>
